...
traverse_folders() {
  ...
  while read -r FOLDER_META; do
    ...
    jq -rc '.Items[] | select(.IsFolder==false) | {Id: .Id, Name: .Name}' "${FOLDER_ID}.json" \
      > "VideosIn${FOLDER_ID}.jsonl"

    jq -rc '.Items[] | select(.IsFolder==true) | {Id: .Id, Name: .Name}' "${FOLDER_ID}.json" \
      > "FoldersIn${FOLDER_ID}.jsonl"
